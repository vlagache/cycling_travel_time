...
def transform_road_to_dataset(road, road_segmented):
    ...
    for i in range(len(start_end_segments.xs('first', level=1))):
        ...
        if i == 0:
            all_points_segment = road[road['timestamp'].between(start_end_segments.xs('first', level=1)['timestamp'][i],
                                                                start_end_segments.xs('last', level=1)['timestamp'][i])]
            ...
            duration = (all_points_segment['timestamp'].tail(1).values[0] -
                        all_points_segment['timestamp'].head(1).values[0]) / np.timedelta64(1, 's')
            ...
            gain_altitude = round(
                (all_points_segment.tail(1)['altitude'].values[0] - all_points_segment.head(1)['altitude'].values[0]),
                2)
            distance = all_points_segment.tail(1)['distance'].values[0]
            denivele = round(((gain_altitude * 100) / distance), 2)

            infos_segment = []
            infos_segment.extend((date, duration, mean_power, mean_speed, mean_heart_rate, mean_cadence, distance,
                                  gain_altitude, denivele))
        ...
        # Logique pour tout les autres segments
            ...
            all_points_segment = road[
                road['timestamp'].between(start_end_segments.xs('last', level=1)['timestamp'][i - 1],
                                          start_end_segments.xs('last', level=1)['timestamp'][i])]
            all_points_segment['timestamp'] = pd.to_datetime(all_points_segment['timestamp'])
            date = pd.to_datetime(all_points_segment['timestamp'].head(1).values[0]).date()
            duration = (all_points_segment['timestamp'].tail(1).values[0] -
            ...
            gain_altitude = round(
                ...
                2)
            distance = round(
                (all_points_segment.tail(1)['distance'].values[0] - all_points_segment.head(1)['distance'].values[0]),
                2)
            denivele = round(((gain_altitude * 100) / distance), 2)
